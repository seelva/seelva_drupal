...
function seelva_form_system_theme_settings_alter(&$form, $form_state) {
  ...
  **/
  ...
  if (!empty($seelva_stylesheets['stylesheets'])) {
    ...
    foreach ($seelva_stylesheets as $item) {
      // Stylesheets are represented as per-media arrays: print, screen...
  ...
  */
  ...
  $form['google_fonts']['enable_google_fonts'] = array(
    '#type'          => 'checkbox',
    '#title'         => t('Use fonts from Google Fonts'),
    '#default_value' => theme_get_setting('enable_google_fonts'),
  );

  $form['google_fonts']['google_fonts_list'] = array(
  	'#title' => t('Google Fonts'),
  	'#type' => 'textarea',
  	'#description' => 'List of fonts From Google Fonts to be included in your theme. One URL per line.',
  	'#default_value' => theme_get_setting( 'google_fonts_list' )
  );
}
